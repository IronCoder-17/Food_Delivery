...
The following diagram shows the overall architecture of the QuickBite food delivery platform, including the React web portals, Flask API, domain services, data layer, external services, and user roles.
...
(Optional, for the AI assistant / recipe-to-order features) A free OpenRouter API key
...
#                                                see section 8 "Real Email OTP Setup")
...
Create or use a Gmail account for the application.

Turn on 2-Step Verification on that Google account:
myaccount.google.com/security.

Once 2-Step Verification is on, go to
myaccount.google.com/apppasswords and create an
App Password (choose "Mail" / "Other", give it a name like "QuickBite backend"). Google
gives you a 16-character code — copy it.

Put the App Password into MAIL_PASSWORD in backend/.env, and fill in the rest:
...
Never commit backend/.env, never put the App Password (or any MAIL_* value) in
frontend/.env or any Vite (VITE_*) variable, and never push it to GitHub. Only the Flask
backend ever reads MAIL_PASSWORD.

Start the backend (python app.py) and frontend (npm run dev).

Test registration: /register → fill in the form → Send OTP next to Email Address → check
...
Test Forgot Password: /login → Forgot password? → enter a real, registered email →
...
Uses an OpenAI-compatible /chat/completions endpoint via OpenRouter by
